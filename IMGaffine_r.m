...
N = OSIZE(2);
 ...
 P(1,:) = reshape( (1:M)'*ones(1,N), M*N, 1)'; % generate 1,2...,M,1,2,..,M
 P(2,:) = reshape( ones(M,1)*(1:N), M*N, 1)';  % generate 1,1,..,1,2,2,..2,...,N,N...N
 P(3,:) = ones(1, M*N);
...
j = find(Q(1,:)>=1 & Q(1,:)<=M_in & Q(2,:)>=1 & Q(2,:)<N_in);  %find the index in the first and second row satisfy such condition
 
if length(j)>0  % number of indices bigger than 0
  in_range = 1;
  R_out1(j) = R_in((Q(2,j)-1)*M_in+Q(1,j)); % set the values to the colume
  a = mean(R_out1(j));
  R_out1(setdiff(1:M*N, j)) = a;  % set other index equal to the mean
